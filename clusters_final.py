...
def parse_arguments():
    ...
    help_im = "minimum cluster size"
    required.add_argument("-min_size", "--min_size", help=help_im, type=int, required=True)
    ...
    if not os.path.isdir(args.statistics_dir):
    	os.makedirs(ags.statistics_dir)
...
def get_clusters(data, e = 35, min_size = 6, clusterMethod='dbscan'):
	if clusterMethod == 'dbscan':
		...
		clust = HDBSCAN(min_cluster_size=min_size, allow_single_cluster=False).fit(data)
	else: sys.exit("ERROR")
...
def clusterInfo(data, labels, out_path, h, w):
...
def extract_clusters(user_options):
	for csv_name in os.listdir(user_options["input_res"]):
		if csv_name.split(".")[-1] in ["csv"]:
			...
			stats_path = 'clusterHighResCSV/' + csv_name
			...
			clusterInfo(coordinates, labels, stats_path, h ,w)
